...
func Initialize(address string, healthCheck *HealthCheck) {
	go func() {
		http.Handle("/metrics", promhttp.Handler()) //为Prometheus注册HTTP服务端，给/metrics路径提供服务
		if healthCheck != nil {
			http.Handle("/health-check", healthCheck) //在判断health-check是否正常后，给health-check提供http服务
		}
		err := http.ListenAndServe(address, nil)//监听Prometheus地址，接收8944端口发送过来的信息
