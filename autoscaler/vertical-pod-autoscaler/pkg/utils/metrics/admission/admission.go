...
type AdmissionStatus string  //Admission Control执行状态

// AdmissionResource describes the resource processed by Admission Control execution
type AdmissionResource string //Admission Control执行的资源类型
...
func Register() {
	prometheus.MustRegister(admissionCount) // 注册admission的pod数量
	prometheus.MustRegister(admissionLatency) // 注册admission的延迟
}
