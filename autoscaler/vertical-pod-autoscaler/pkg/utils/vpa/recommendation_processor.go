...
// RecommendationProcessor对recommendation进行后处理，以根据限制和环境前后关系进行调整
type RecommendationProcessor interface {
	...
	// 基于container的限制，Apply处理和更新给定pod的recommendation，VPA策略或者是内部RecommendationProcessor的前后关系
	// 必须返回一个非null的指针指向RecommendationPodResources或错误。
