...
func NewAllVpasLister(vpaClient *vpa_clientset.Clientset, stopChannel <-chan struct{}) vpa_lister.VerticalPodAutoscalerLister {
	...
	//调用了cache的NewListWatchFromClient函数，cache的NewListWatchFromClient函数创建了一个新的ListWatch，用于客户端和资源之间的数据交互。
	...
	//调用cache.NewIndexerInformer函数，通过sharedIndexInformer结构体创建了属于vpa自己的informer框架。
	//这里的informer起到和API Servers建立watch和list长连接的作用。
	//返回的两个变量indexer和controller的作用是提供了资源缓存的功能和控制管理vpa
	vpaLister := vpa_lister.NewVerticalPodAutoscalerLister(indexer)
	//通过indexer的资源缓存功能来获得所有的vpa信息。
	go controller.Run(stopChannel) //协程启动controller来管理vpa
...
// 比较哪个vpa创建的时间比较早，早的那个作为最好的vpa(这尼玛也太简单了吧？？？？)
// 时间相等（一般只在测试环境中出现），按照字符顺序比较名字大小
...
// GetControllingVPAForPod从输入列表中选择与给定Pod匹配的最早创建的VPA。
...
// 返回给定vpa的更新策略，如果指定mode，则返回默认UpdateModeAuto
