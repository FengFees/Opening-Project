...
const (
	// RecommendationMissingMaxDuration是我们能接受的没有推荐值的最大时长
	RecommendationMissingMaxDuration = 30 * time.Minute
)


// ClusterState中保存了VPA操作所需的所有集群运行时信息，即资源配置(pods，容器，VPA对象)，
// 计算资源的综合利用率(CPU，内存)和事件(容器OOM)
// 所有VPA Recommender算法的输入都在这个类中。
type ClusterState struct {
	// 集群中的容器
	Pods map[PodID]*PodState
	// 集群中的VPA对象
	Vpas map[VpaID]*Vpa
	// 键值为没有推荐值的集群中的VPA对象。对应数值为第一次发现没有推荐值的时间
	// 或者最后一次为它记录警告的时间
